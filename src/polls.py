...
def wiki_polls_preprocessing(df, col_names = {
    ...
    df[col_names["Oth"]] = df.apply(lambda x: calculate_others([x[col_names[party]] for party in col_names.keys() if party!="Oth"]) if x[col_names["Oth"]]==9.99 else x[col_names["Oth"]],axis=1)
    for col in df.columns:
        if type(df[col].iloc[0]) is str:
...
def fetch_html(url):
    time.sleep(0.5)
    headers = {
        "User-Agent": "ElectionModelsPollingBot/0.0 (electionmodels@gmail.com)"
    }
